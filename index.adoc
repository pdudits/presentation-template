...
[source, cypher]
----
match p=(m:Method{name:"com.rwe.icon.si.cr.schedule."+
        "CPBIProcessingScheduler#process()V"})
        -[:invokes|overriden*..14]->(m2{public:true})<-[:hasMethod]-(c:Class)
...
* https://github.com/structgraph/structgraph
* GPL Licensed
